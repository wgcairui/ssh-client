...
- **多标签页**: 支持最多10个并发SSH连接，通过标签页切换
- **连接管理**: 保存和重用SSH连接配置
- **会话保持**: SSH连接在后台保持活跃，避免服务器超时断开
- **智能焦点**: 优化的键盘输入处理，确保输入正确路由到活跃终端
- **连接历史**: 自动记录最近使用的连接
...
1. 前往 [Releases页面](https://github.com/cairui/ssh-client/releases) 
2. 选择版本类型：
   - **正式版本**: 稳定可靠，推荐日常使用
   - **开发版本**: 包含最新功能，标记为 `Pre-release`
3. 下载APK文件：
   - `app-release.apk` - 通用版本，**推荐所有用户**
   - `app-arm64-v8a-release.apk` - ARM64优化版（现代设备）
   - `app-armeabi-v7a-release.apk` - ARM32兼容版（老设备）
4. 在Android设备上安装APK文件

#### 方式二：自动发布
- **main分支更新时**: 自动生成开发版本到Releases
- **创建版本标签时**: 自动生成正式版本到Releases
...
2. 系统会自动创建新的标签页并建立连接
3. 连接成功后可以执行各种SSH命令
4. 可以同时打开多个连接，通过标签页切换使用

### 多标签页管理
- **新建连接**: 选择新的服务器会自动创建标签页，最多支持10个
- **标签切换**: 点击标签页可以在不同连接间切换
- **关闭标签**: 点击标签页的关闭按钮可以断开连接并关闭标签
- **连接复用**: 如果选择已打开的连接，会直接切换到对应标签页
